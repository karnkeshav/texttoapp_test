...
const { pooledStream, pooledGenerate, generateImage } = require('../services/geminiPool');
...
function classifyTopLevelIntent(message) {
  ...
  // Image generation — check BEFORE build (create/make can appear in both)
  // But only when not paired with a build noun (handled by ordering)
  if (IMAGE_GEN_RE.test(message) && !BUILD_SIGNAL_RE.test(message)) return 'imageGen';
...
• For generating NEW images from a text prompt, the user can say "generate an image of X" and
  Ready4Launch will use Google Imagen 3 automatically.
...
// System instruction for image generation confirmations
const SYS_IMAGE_GEN = `You are Ready4Launch's image assistant. An image has just been generated by Google Imagen 3 based on the user's prompt — it will appear below your message.
Write 1-2 sentences acknowledging the result: describe what was created and invite the user to refine it with a follow-up prompt (e.g. "make it more vibrant", "add a sunset glow", "change the style to watercolour").
Do NOT claim you cannot generate images. Do NOT output REPO_NAME or HTML code blocks.`;
...
router.post('/chat', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (isFirstMessage && !isEditMode) {
      ...
      if (intent === 'imageGen') {
        req.session.chatPhase = 'imageGen';
        req.session.chatHistory.push({ role: 'user', content: trimmedMessage });
        sendEvent('status', { message: 'Generating image with Google Imagen 3…' });
        try {
          const { base64, mimeType } = await generateImage(trimmedMessage, apiKey);
          const caption = `Here's your generated image! I used Google Imagen 3 based on your prompt. You can refine it — try "make it more vibrant", "change the style to oil painting", or describe any adjustments.`;
          req.session.chatHistory.push({ role: 'assistant', content: caption });
          sendEvent('chunk', { text: caption });
          sendEvent('done', { text: caption, imageGenerated: true, imageData: { base64, mimeType } });
        } catch (imgErr) {
          const errMsg = `⚠️ Image generation failed: ${imgErr.message}`;
          req.session.chatHistory.push({ role: 'assistant', content: errMsg });
          sendEvent('chunk', { text: errMsg });
          sendEvent('done', { text: errMsg });
        }
        return res.end();
      }
    ...
    if (!isFirstMessage && ['conversion', 'reasoning', 'chat', 'vision', 'imageGen'].includes(req.session.chatPhase)) {
      ...
      if (BUILD_SIGNAL_RE.test(trimmedMessage) && !IMAGE_GEN_RE.test(trimmedMessage)) {
        ...
        if (IMAGE_GEN_RE.test(trimmedMessage) && !BUILD_SIGNAL_RE.test(trimmedMessage)) {
          req.session.chatPhase = 'imageGen';
        } else if (CONVERSION_RE.test(trimmedMessage)) {
          req.session.chatPhase        = 'conversion';
          req.session.conversionFormat = detectConversionFormat(trimmedMessage);
        } else if (REASONING_RE.test(trimmedMessage)) {
          req.session.chatPhase = 'reasoning';
        } else if (req.session.chatPhase === 'vision' || req.session.chatPhase === 'imageGen') {
          // vision/imageGen sessions stay in their mode unless re-routed above
          // (the user uploaded an image or was generating images — keep context)
        } else {
        ...
        // ── Image generation ─────────────────────────────────────────────────
        if (req.session.chatPhase === 'imageGen') {
          sendEvent('status', { message: 'Generating image with Google Imagen 3…' });
          try {
            const { base64, mimeType } = await generateImage(trimmedMessage, apiKey);
            const caption = `Here's your updated image! You can keep refining — describe any changes you'd like.`;
            req.session.chatHistory.push({ role: 'assistant', content: caption });
            sendEvent('chunk', { text: caption });
            sendEvent('done', { text: caption, imageGenerated: true, imageData: { base64, mimeType } });
          } catch (imgErr) {
